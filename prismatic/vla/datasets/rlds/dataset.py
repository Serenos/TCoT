...
def make_dataset_from_rlds(
    ...
    *,
    ...
    reasoning_dataset_path: str = "/home/lixiang/codebase/embodied-CoT/datasets/libero/embodied_features_libero_spatial.json", #"~/.cache/reasonings_dataset.json",
    trajectory_tcot: bool = True,
    ...
    if  "fail" not in name and trajectory_tcot:
        traj_dataset_path = f'/home/lixiang/codebase/embodied-CoT/scripts/generate_trajectory/trajectory_data/full_trajectory_v3.1_{name}.json' #v4 [x,y,depth,gripper_state]
        if os.path.isfile(traj_dataset_path):
            print(f"Loading from local checkpoint path `{traj_dataset_path}`.............................")
            with open(traj_dataset_path, "r") as f:
                trajectory_dataset = json.load(f)
        else:
            print("No trajectory file loaded..............................................................")
    ...
    def make_tf_global_traj_dict(raw_dict, name):
        print("Building the globla trajectory dict...")
        keys = []
        values = []

        has_trajectory = [0, 0]
        full_trajectory = True #whether use past trajectory
        #210 for libero_goal #150 for libero spatial/object  #300 for libero10 #
        if 'spatial' in name:
            suite = 'libero_spatial'
            global_horizon = 150 
            global_sampeling_interval = 5
        elif 'object' in name:
            suite = 'libero_object'
            global_horizon = 150 
            global_sampeling_interval = 5
            ...
            suite = 'libero_goal'
            global_horizon = 210 
            global_sampeling_interval = 7
        elif '10' in name:
            suite = 'libero_o10'
            global_horizon = 300 
            global_sampeling_interval = 10
        else:
            suite = 'airbot'
            global_horizon = 90 
            global_sampeling_interval = 3 #7 #5 #10 #
        print(f'dataset: {name}, suite: {suite}')

        for file_name in raw_dict.keys():
            for episode_id in raw_dict[file_name].keys():
                if "trajectory_points" not in raw_dict[file_name][episode_id].keys():
                    has_trajectory[0] += 1
                    continue
                else:
                    has_trajectory[1] += 1

                trajectory_points = raw_dict[file_name][episode_id]["trajectory_points"]
                dim = len(trajectory_points[0])
                if dim==2:
                    trajectory_points = [[int(p) for p in pair] for pair in trajectory_points]
                else:
                    trajectory_points = [[int(p) if i!=3 else int(255*p) for i, p in enumerate(pair)] for pair in trajectory_points]
                ...
                for i in range(len(trajectory_points)):
                    ...
                    trajectory = []
                    for j in range(global_horizon//global_sampeling_interval):
                        if j*global_sampeling_interval <  len(trajectory_points):
                            trajectory += trajectory_points[j*global_sampeling_interval]
                        else:
                            trajectory += trajectory[-dim:] #-2
                    values.append(str(trajectory))

        print(f"global_trajectory, horizon={global_horizon}, interval={global_sampeling_interval}, full_trajectory={full_trajectory}")
        print("Example trajectory:", keys[0], values[0])
        print("Trajectory presence statistics [# has not, # has]:", has_trajectory)

        return tf.lookup.StaticHashTable(tf.lookup.KeyValueTensorInitializer(keys, values), default_value="")

    def make_tf_local_traj_dict(raw_dict, name):
        print("Building the local_trajectory dict...")
        keys = []
        values = []

        has_trajectory = [0, 0]

        full_trajectory = False #whether use past trajectory
        long_trajectory = True #whether use long horizon 150 or 30

        #local_trajectory
        local_horizon = 10
        local_sampeling_interval = 2
        ...
        for file_name in raw_dict.keys():
            for episode_id in raw_dict[file_name].keys():
                ...
                for i in range(len(trajectory_points)):
                    keys.append(file_name + "_" + str(episode_id) + "_" + str(i))
                    trajectory = []
                    for j in range(local_horizon//local_sampeling_interval):
                        if i + j*local_sampeling_interval <  len(trajectory_points):
                            trajectory += trajectory_points[i + j*local_sampeling_interval]
                        else:
                            trajectory += trajectory[-dim:]
                    values.append(str(trajectory))

        print(f"local_trajectory, horizon={local_horizon}, interval={local_sampeling_interval}, full_trajectory={full_trajectory}")
        print("Example trajectory:", keys[0], values[0])
        print("Trajectory presence statistics [# has not, # has]:", has_trajectory)
    ...
    if  "fail" not in name and trajectory_tcot:
        global_trajectory_dataset = make_tf_global_traj_dict(trajectory_dataset, name)
        local_trajectory_dataset = make_tf_local_traj_dict(trajectory_dataset, name)
    ...
    def restructure(traj):
        ...
        if  "fail" not in name and trajectory_tcot:
            episode_id = traj["traj_metadata"]["episode_metadata"]["episode_id"][0] #for bridge
            episode_ids = tf.as_string(tf.repeat(episode_id, traj_len))
        # episode_id = traj["_traj_index"][0] #for libero
        # print('traj["_traj_index"][0]:', episode_id)
        ...
        #print(name)
        if  "fail" not in name and trajectory_tcot:
            g_trajectory = global_trajectory_dataset.lookup(file_names + "_" + episode_ids + "_" + indices)
            l_trajectory = local_trajectory_dataset.lookup(file_names + "_" + episode_ids + "_" + indices)
        else:
            g_trajectory = tf.repeat(name, traj_len) #for padding
            l_trajectory = tf.repeat(name, traj_len) #for padding
        traj = {
            "observation": new_obs,
            "task": task,
            ...
            "global_trajectory": g_trajectory,
            "local_trajectory": l_trajectory,
            "reward": tf.cast(traj["reward"], tf.float32),
            "is_terminal": tf.cast(traj["is_terminal"], tf.bool)
        }

        # if trajectory_tcot:
        #     traj["global_trajectory"] = g_trajectory
        #     traj["local_trajectory"] = l_trajectory
        # if "libero" in name:
        #     traj["reward"] = tf.cast(traj["reward"], tf.float32)
        #     traj["is_terminal"] = tf.cast(traj["is_terminal"], tf.bool)
        #     traj["global_trajectory"] = g_trajectory,
        #     traj["local_trajectory"] = l_trajectory,
    ...
    if "val" not in builder.info.splits:
        ...
        ## for ablation study
        #split = "train[:75%]" if train else "train[75%:]"
        #split = "train[:55%]" if train else "train[55%:]"
        split = "train[:50%]" if train else "train[50%:]"
    else:
