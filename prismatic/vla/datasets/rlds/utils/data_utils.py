...
# === Airbot-Mix =>> Dataset-Specific Transform ===
def relabel_airbot_actions(traj: Dict[str, Any]) -> Dict[str, Any]:
    """Relabels actions to use reached proprioceptive state; discards last timestep (no-action)."""
    step = 1 #origin step=1, frame1 predict frame4 action

    movement_actions = traj["action"][step:, :6] - traj["action"][:-step, :6]
    traj_truncated = tf.nest.map_structure(lambda x: x[:-step], traj)
    traj_truncated["action"] = tf.concat([movement_actions, traj["action"][:-step, -1:]], axis=1)
