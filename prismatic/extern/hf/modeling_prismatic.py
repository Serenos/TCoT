...
class TcotSearchForActionPrediction(PrismaticForConditionalGeneration):
    config_class: PretrainedConfig = OpenVLAConfig
    ...
    def __init__(self, config: OpenVLAConfig) -> None:
        ...
        # Compute vocab size for de-tokenization -- revert added "multiple of"
        self.vocab_size = self.config.text_config.vocab_size - self.config.pad_to_multiple_of
        self.freezing_cot = True
        self.base_prompt = "Trajectory:"

        self.max_freezing_time = 10
        self.time_frozen = 0
        self.frozen_prompt = self.base_prompt


    def predict_action(
        self, input_ids: Optional[torch.LongTensor] = None, unnorm_key: Optional[str] = None, tcot_status=True, fail_det=False, **kwargs: str
    ) -> np.ndarray:
        """Thin wrapper around super().generate() that decodes predicted actions and de-normalizes them."""
        if fail_det:
            generated_ids = self.generate(input_ids, max_new_tokens=1, **kwargs)
            return generated_ids[0]
        # Run VLA inference
        # Extract predicted action tokens and translate into (normalized) continuous actions
        if tcot_status:
            generated_dicts = self.generate(input_ids, **kwargs)
            generated_ids = generated_dicts.sequences
            ...
            generated_dicts = self.generate(input_ids, max_new_tokens=self.get_action_dim(unnorm_key), **kwargs)
            generated_ids = generated_dicts.sequences
            # Extract predicted action tokens and translate into (normalized) continuous actions
            predicted_action_token_ids = generated_ids[0, -self.get_action_dim(unnorm_key) :].cpu().numpy()
        ...
        return actions, generated_ids, generated_dicts
